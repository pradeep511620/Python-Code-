...
for url in Result:
    ...
    # urlss = driver.find_element(by=By.XPATH, value='//*[@id="specs-body"]/div/div/div[3]/div[2]')
    # u = urlss.find_elements(by=By.TAG_NAME, value='a')
    # for i in u:
    #     print(i.get_attribute('href'))

    ur = driver.find_element(by=By.TAG_NAME, value='div').find_elements(by=By.CLASS_NAME, value='product-wrapper')
    for x in ur:
        # time.sleep(2)
        if x.get_attribute('onclick') != "window.location='":
            link = x.get_attribute('onclick')
            print(link)
