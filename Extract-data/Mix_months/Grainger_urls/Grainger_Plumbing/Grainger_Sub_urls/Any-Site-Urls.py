...
def extract_urls_recursive(url, visited=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # print(soup)
            # try:
            product_links = soup.find(class_="-jeeqs").findAll('li')
            # print(product_links)
        ...
        print(e)
    print(urls)
    save = open('grainger11.txt', 'a+', encoding='utf-8')
    save.write('\n' + "'" + str(urls) + "',")
    return urls


cat_url = 'https://www.grainger.com/category/plumbing'

product_urls = extract_urls_recursive(cat_url)
print(product_urls)
print(len(product_urls))
save = open('granger_urls_pipe-hose-tube-fittings.csv', "a+", encoding="utf-8")
