...
product_data = open("stearn_brakes_mroSupplies_product_data3.csv", 'a', encoding='utf-8')
# product_specs = open("/var/www/html/web_scr/scrapped_data/stearn_brakes_mroSupplies__specs.csv", 'a', encoding='utf-8')
product_specs = open("stearn_brakes_mroSupplies__specs3.csv", 'a', encoding='utf-8')
    ...
    try:
        ...
        for data in specs:
            for dt, dd in zip(data.findAll(class_='flex-table--head'), data.findAll(class_='flex-table--body')):
                attr_name.append(str(dt.text.strip()))
...
def main():
    ...
    for url in product_urls[i:15000]:   # start point 10000 to 15000
        print(i, ">>>>>>>>>>>>>", url)
        try:
