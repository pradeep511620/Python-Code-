...
base_url = 'https://www.lighting.philips.com/'
...
def extract_urls_recursive(url, visited=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # print(soup)
            try:
                product_links = soup.find('div', class_='aem-Grid aem-Grid--12 aem-Grid--default--12 ').find_all('li')
                print("......................", product_links)
                for links in product_links:
                    absolute_url = url_join_func(links.find('a')['href'])
                    print(absolute_url)
            except AttributeError:
                product_links = soup.findAll(class_="product-detail-container")
        ...
        print(e)
    print(urls)
    save = open('philips.txt', 'a+', encoding='utf-8')
    save.write('\n' + "'" + str(urls) + "',")
    return urls


cat_url = 'https://www.lighting.philips.com/home'

product_urls = extract_urls_recursive(cat_url)
print(product_urls)
print(len(product_urls))
save = open('philips.csv', "a+", encoding="utf-8")
