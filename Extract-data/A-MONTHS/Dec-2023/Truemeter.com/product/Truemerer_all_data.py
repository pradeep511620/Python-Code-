...
save_files = open("C:/Users/PK/Desktop/web-scrapping/A-MONTHS/Dec-2023/Truemeter.com/data/Truemeter-all-data.csv", 'a',
                  encoding='utf-8')
...
def product_detail(url, soup):
    ...
    # ------------------------------------------------------------------------------------------------------------------
    try:
        sku = soup.find_all('div', class_="elementor-widget-container")[47].text.strip().replace("\n", " >> ").encode('latin-1', errors='ignore').decode('utf-8', errors='ignore')
        # sku = soup.find('div', {"data-id": "65a656b6"}).text.strip().replace('\n', " >> ").encode('latin-1', errors='ignore').decode('utf-8', errors='ignore')
        print('sku.....', sku)
    except AttributeError:
        sku = "N/A"
        print('sku.....', sku)
    ...
    # ------------------------------------------------------------------------------------------------------------------
    try:
        short_description = soup.find('div',
                                      {"class": "woocommerce-product-details__short-description"}).text.strip().replace(
            '\n', " >> ")
    ...
    # ------------------------------------------------------------------------------------------------------------------
    try:
        feature = soup.find_all('div', class_="elementor-widget-container")[42].text.strip().replace('\n', " >> ")
    ...
    try:
        description = ''
        desc = soup.find_all('div', class_="elementor-widget-container")[44]
        for lists in desc:
    ...
    try:
        ...
        for img_link in img_links:
            # print(img_link)
            href = img_link.get('data-large_image')
            image.append(href)
    ...
    try:
        datasheet = []
        pdf_link = soup.find("div", {"class": "jupiterx-widget widget_wc_product_documents_widget_documents"}).find_all(
            'a')
        # print(pdf_link)
        for pdf in pdf_link:
...
def Get_Specs_Table(url, soup, product_title, sku):
    ...
    #   /var/www/html/webscr/destination_dir
    save = open("C:/Users/PK/Desktop/web-scrapping/A-MONTHS/Dec-2023/Truemeter.com/data/Truemeter-specs.txt", 'a+',
                encoding='utf-8')
    print('Tables')
    ...
    for th in soup("th", class_='woocommerce-product-attributes-item__label'):
        tab = th.text.strip()
        attr_name.append(tab)

    for td in soup("td", class_='woocommerce-product-attributes-item__value'):
        tab_1 = td.text.strip()
        attr_value.append(tab_1.encode('latin-1', errors='ignore').decode('utf-8', errors='ignore'))
...
def main():
    ...
    url_link = \
    pd.read_csv("C:/Users/PK/Desktop/web-scrapping/A-MONTHS/Dec-2023/Truemeter.com/url/Truemeter-product-url.csv")[
        'URL']
