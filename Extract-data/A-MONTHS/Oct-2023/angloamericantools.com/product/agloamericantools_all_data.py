...
save_files = open(
    "C:/Users/PK/Desktop/web-scrapping/A-MONTHS/Oct-2023/angloamericantools.com/data/angloamericantools-all-data.csv",
    'a', encoding='utf-8')
...
def product_detail(url, soup):
    ...
    l3_name = []
    bread = soup.find_all('ul', {"id": "breadcrumbs"})
    for l3 in bread:
    ...
    try:
        product_title = soup.find('h1', id="prodName").text.strip()
    ...
    description = []
    for desc in soup.find('div', {"class": "textBlock"}):
    ...
    try:
        image = []
        for img_1 in soup.find_all('table', {"id": "product"}):
...
def get_specs(url, soup, product_title, mpn):
    print('Tables')
    try:
        count_tab = 0
        attr_name = []
        attr_value = []
        for th in soup.find('div', {"id": "textSpecBlock"}).find_all('table'):
        ...
        for a, b in zip(attr_name, attr_value):
            print(a, "..........", b)
            save = open("C:/Users/PK/Desktop/web-scrapping/A-MONTHS/Oct-2023/angloamericantools.com/data/angloamericantools-specs.txt",
                        'a+', encoding='utf-8')
...
def main():
    ...
    url_link = pd.read_csv(
        "C:/Users/PK/Desktop/web-scrapping/A-MONTHS/Oct-2023/angloamericantools.com/url/angloamericantools_product_url.csv")[
        'url']
    for url in url_link[11:]:
        url_link_count += 1
        print("Product-Length...", url_link_count)
        print("Product-Urls......", url)
        soup = Get_Soup_Url(url)
        product_title, mpn = product_detail(url, soup)
        get_specs(url, soup, product_title, mpn)
