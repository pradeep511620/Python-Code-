...
save_files = open("C:/Users/PK/Desktop/web-scrapping/A-MONTHS/Sep-2023/wlenk.com/data/wlenk-all-data.csv", 'a',
                  encoding='utf-8')
...
def product_detail(url, soup):
    ...
    try:
        category = soup.find('span', {"class": "posted_in"}).text.strip()
    ...
    try:
        features = soup.find('div', class_="woocommerce-product-details__short-description").find('p')
        features_1 = features.text
    ...
    try:
        ...
        for pdf in soup.find_all('div', {"class": "desc"}):
            for h4_tag in pdf.find_all('h4'):
                a_tag = h4_tag.find('a')
                if a_tag:
                    related_product_urls.append(a_tag.get('href'))
        print('datasheet.....', related_product_urls)
    except AttributeError:
        related_product_urls = ''
    ...
    try:
        image = []
        for img_1 in soup.find('div', {"class": "product_image_wrapper"}).find_all('img'):
            srcs = img_1.get('src')
            image.append(srcs)
        # print("Images....", image)
    except AttributeError:
    ...
    row = [{
        ...
        "Features": features_1,
        "Accessories": related_product_urls,
        # "Datasheet": datasheet,
        # "item_ID": item,
        "Uses": price,
        # "Video_Url": video,
...
def get_specs(url, soup, sku):
    ...
    try:
        attr_name = []
        attr_value = []
        for ths in soup.find_all(class_="woocommerce-product-attributes-item__label"):
            attr_name.append(ths.text.strip())
        for tds in soup.find_all(class_="woocommerce-product-attributes-item__value"):
            attr_value.append(tds.text.strip())
        for a, b in zip(attr_name, attr_value):
            print(a, "..........", b)
            save = open("C:/Users/PK/Desktop/web-scrapping/A-MONTHS/Sep-2023/wlenk.com/data/wlenk_specs.txt", 'a+', encoding='utf-8')
            save.write('\n' + url + " \t" + sku + "\t" + a + "\t" + b)
...
def main():
    url_link_count = 20
    url_link = pd.read_csv("C:/Users/PK/Desktop/web-scrapping/A-MONTHS/Sep-2023/wlenk.com/url/wlenk_product_url.csv")['url']
    for url in url_link[20:]:
        url_link_count += 1
