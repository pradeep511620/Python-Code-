...
save_files = open("C:/Users/PK/Desktop/web-scrapping/A-MONTHS/Oct-2023/modifyair.com/data/modifyair-all-data.csv", 'a', encoding='utf-8')
...
def product_detail(url, soup):
    # try:
        ...
        # ------------------------------------------------------------------------------------------------------------------
        try:
            product_title = soup.find('div', {"class": "product-page-info__title"}).h1.text.strip()
        ...
        try:
            datasheet = []
            pdf = soup.find('div', {"class": "text-center mt-10"}).find_all('a')
            for pdf_link in pdf:
        ...
        try:
            image = []
            table = soup.find('div', {"class": "product-gallery__content"})
            img_links = table.find_all('img')
            for img_link in img_links:
                href = img_link.get('data-srcset').replace("//", "")
                image.append(href)
            while len(image) < 5:
                image.append('')
...
def Get_Specs_Table(url, soup, product_title):
    #   /var/www/html/webscr/destination_dir
    save = open("C:/Users/PK/Desktop/web-scrapping/A-MONTHS/Oct-2023/modifyair.com/data/modifyair-specs.txt", 'a+', encoding='utf-8')

    print('Tables')

    attr_name = []
    attr_value = []
    tables = soup.find_all('div', {"class": "product-spec__table"})
    for table in tables:
        for th in table.find_all('th'):
            tab = th.text.strip().replace("\n", "<<<")
            attr_name.append(tab)
        for td in table.find_all('td'):
            tab_1 = td.text.strip().replace("\n", "<<<")
