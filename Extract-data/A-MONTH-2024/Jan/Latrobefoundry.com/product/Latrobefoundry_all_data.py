...
save_files = open(
    "C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Jan/Latrobefoundry.com/data/Latrobefoundry-all-data.csv", 'a',
    encoding='utf-8')
...
def product_detail(url, soup):
    ...
    try:
        l3_name = []
        l3_name = soup.find("nav", {"id": "plp-bread-crumb"}).text.strip().split("All CategoriesAll")[0]
        # print(l3_name)
        # for l3 in bread:
    ...
    # ------------------------------------------------ Title -----------------------------------------------------------
    try:
        product_title = soup.find("nav", {"id": "plp-product-title"}).h1.text.strip().replace("\n", ">>")
    ...
    description = []
    try:
        for descriptions in soup.find(class_="plp-item-description").find("ul").find_all('li'):
            des = descriptions.text.strip().replace("\n", ">>")
    ...
    try:
        ...
        for img_tag in soup.find("div", {"id": "imageCarousel"}).find_all("img"):
            ...
            image.append("https://fittings.latrobefoundry.com" + href)
...
def Get_Specs_Table(url, soup, product_title):
    #   /var/www/html/webscr/destination_dir
    save = open("C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Jan/Latrobefoundry.com/data/Latrobefoundry-specs.txt", 'a+', encoding='utf-8')
    print('Tables ****************************************************************************************************')
    count = 0
    attr_name = []
    attr_value = []
    for table in soup.find("table", {"class": "plp-table"}).find_all('td'):
...
def main():
    ...
    file_path = "C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Jan/Latrobefoundry.com/url/Product-url.csv"
