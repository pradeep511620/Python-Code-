...
save_files = open("C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Jan/Lalizas.com/data/Lalizas-all-data.csv", 'a',
                  encoding='utf-8')
...
def product_detail(url, soup):
    ...
    try:
        l3_name = []
        bread = soup.find("ul", {"class": "breadcrumbs fixclear"}).find_all('li')
        for l3 in bread:
    ...
    # ------------------------------------------------ Title -----------------------------------------------------------
    try:
        product_title = soup.find("div", {"class": "header-titles"}).h1.text.strip()
    ...
    try:
        description = []
        for des in soup.find("div", {"class": "small_desc"}).find_all("p"):
    ...
    try:
        logo = []
        for logo_tag in soup.find("div", {"class": "small_desc"}).find_all("img"):
            logo_link = logo_tag.get('src')
            logo.append("https://www.lalizas.com/" + logo_link)
        print('logo.....', logo)
    except (AttributeError, IndexError):
        logo = "N/A"
        print('logo.....', logo)
    ...
    try:
        image = []
        for img_tag in soup.find("div", {"class": "product-gallery"}).find_all("a"):
            href = img_tag.get('href')
            image.append("https://www.lalizas.com/" + href)
    ...
    try:
        datasheet = []
        for pdf in soup.find("div", {"id": "tab-product_documents"}).find_all("a"):
            pdf_tag = pdf.get("href")
            datasheet.append("https://www.lalizas.com/" + pdf_tag)
...
def Get_Specs_Table(url, soup, product_title):
    #   /var/www/html/webscr/destination_dir
    save = open("C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Jan/Lalizas.com/data/Lalizas-specs.txt", 'a+', encoding='utf-8')
    print('Tables ****************************************************************************************************')
    table_img_tag = []
    table_row_data = []
    try:
        for table_img in soup.find('table', {"id": "tblItems"}).find('tbody').find_all("img"):
            img_tag_link = table_img.get('src')
            table_img_tag.append(img_tag_link)
        table_img_tag = list(set(table_img_tag))
        # print(table_img_tag)
    except AttributeError:
        print('NN')
    ...
    try:
        for table in soup.find('table', {"id": "tblItems"}).find('tbody').find_all("tr"):
            table_1 = table.text.strip().replace("\n", ">>").split(">>")
            if table_1:
                table_row = "\t".join(table_1)

                table_row_data.append(table_row)
    except AttributeError:
        print('NN')

    for name, value in zip_longest(table_row_data, table_img_tag):
        print(name, "::", value)
...
def main():
    ...
    file_path = "C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Jan/Lalizas.com/url/Product-url.csv"
