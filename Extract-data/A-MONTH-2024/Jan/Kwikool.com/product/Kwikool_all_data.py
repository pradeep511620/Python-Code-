...
save_files = open("C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Jan/Kwikool.com/data/Kwikool-all-data.csv", 'a',
                  encoding='utf-8')
...
def product_detail(url, soup):
    ...
    # ------------------------------------------------ Title -----------------------------------------------------------
    try:
        product_title = soup.find("section", {"class": "page-title"}).h1.text.strip()
    ...
    # ------------------------------------------------------------------------------------------------------------------
    try:
        price = soup.find("p", {"class": "price icon-"}).text.strip()
        print('price.....', price)
    except IndexError:
        price = "N/A"
        print('price.....', price)
    ...
    try:
        description = []
        for des in soup.find("div", {"id": "tab-general-details"}).find_all("p"):
    ...
    try:
        drow_image = []
        for draw in soup.find('div', {'id': 'tab-5'}).find_all('a'):
            draw_tag = draw.get("href")
    ...
    try:
        feature = []
        for product_feature in soup.find('div', {"id": "tab-6"}).find_all("li"):
    ...
    try:
        image = []
        for img_tag in soup.find("div", {"class": "product-image-carosel"}).find_all("a"):
...
def Get_Specs_Table(url, soup, product_title, sku):
    #   /var/www/html/webscr/destination_dir
    save = open("C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Jan/Kwikool.com/data/Kwikool-specs.txt", 'a+', encoding='utf-8')
    print('Tables ****************************************************************************************************')
    attr_name = []
    attr_value = []
...
def main():
    ...
    file_path = "C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Jan/Kwikool.com/url/Product-url.csv"
