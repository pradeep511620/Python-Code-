...
save_files = open("C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Jan/Knapeandvogt.com/data/Knapeandvogt-all-data.csv", 'a',
                  encoding='utf-8')
...
def product_detail(url, soup):
    ...
    try:
        l3_name = []
        bread = soup.find("ul", {"class": "store_breadcrumb"}).find_all('li')
        for l3 in bread:
    ...
    # ------------------------------------------------ Title -----------------------------------------------------------
    try:
        product_title = soup.find("div", {"class": "title_text"}).h3.text.strip()
    ...
    try:
        mor_data = []
        for skus in soup.find_all("div", {"class": "sku_value_data"}):
    ...
    try:
        description = []
        for des in soup.find("div", {"class": "description"}).find_all("p"):
    ...
    try:
        image = []
        for img_tag in soup.find("div", {"class": "image_box"}).find_all("img"):
    ...
    try:
        datasheet = []
        for pdf in soup.find("div", {"class": "product_documents"}).find_all("a"):
            pdf_tag = pdf.get("href")
            # if "youtube" not in pdf_tag:
            datasheet.append("https://www.knapeandvogt.com" + pdf_tag)
...
def Get_Specs_Table(url, soup, sku, product_title):
    #   /var/www/html/webscr/destination_dir
    save = open("C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Jan/Knapeandvogt.com/data/Knapeandvogt-specs.txt", 'a+', encoding='utf-8')
    print('Tables ****************************************************************************************************')
    attr_name = []
    attr_value = []
