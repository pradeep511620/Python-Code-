...
urls = []
save_files = open("D:/Web-Scrapping/A-MONTH-2024/May/Partstown.com/data/Partstown.csv", "a+", encoding="utf-8")
base_url = ''
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        try:
            self.driver.find_element(By.XPATH, "(//button[@aria-label='Close'])[2]").click()
            time.sleep(1)
        except (AttributeError, TypeError, IndexError, NoSuchElementException):
            print('not click cross')

        try:
            self.driver.find_element(By.XPATH, "(//div[@id='onetrust-button-group']//button)[3]").click()
            time.sleep(2)
        except (AttributeError, TypeError, IndexError, NoSuchElementException):
            self.driver.find_element(By.XPATH, "(//div[@id='onetrust-button-group']//button)[2]")

        try:
            names = [l3.text.strip() for l3 in self.driver.find_elements(By.XPATH, "//ol[@class='breadcrumb js-breadcrumb']//li")]
            l3_Name = "## ".join(names)
        ...
        # --------------------------------------------------------------------------------------------------------------
        try:
            product_title = self.driver.find_element(By.XPATH, "//div[@class='product-details page-title']//h1").text.strip()
        ...
        # -------------------------------------------------- Price-- ---------------------------------------------------
        try:
            quantity_details = self.driver.find_element(By.XPATH, "//div[@class='product__cell product__val js-pdp-main-qty-value js-product-stocklvl']").text.strip()
        except NoSuchElementException:
            quantity_details = '1'

        try:
            product_price_1 = self.driver.find_element(By.XPATH, "//*[@data-is-listprice='true']").text.strip()
            price = {'list_price': product_price_1, 'qty': quantity_details, 'moq': '1'}
            print('price.....', price)
        except (NoSuchElementException, ElementClickInterceptedException, AttributeError, TypeError, IndexError):
        ...
        # --------------------------------------------------- MPN ------------------------------------------------------
        try:
            mpn = self.driver.find_element(By.XPATH, "//*[@itemprop='sku']").text.strip()
            print('SKU.....', mpn)
        except (NoSuchElementException, ElementClickInterceptedException, AttributeError, TypeError, IndexError):
            mpn = 'N/A'
            print('SKU:', mpn)
        ...
        try:
            desc = [desc_text.text.strip() for desc_text in self.driver.find_elements(By.XPATH, "//*[@itemprop='description']//p")]
            if not desc:
        ...
        # ---------------------------------------------- Description_2 -------------------------------------------------
        try:
            description_2 = [des_1.text.strip() for des_1 in self.driver.find_elements(By.XPATH, "//*[@itemprop='description']//li")]
        ...
        try:
            images = [{"src": img.get_attribute('src'), "alt": img.get_attribute('alt')} for img in
                      self.driver.find_elements(By.XPATH, "//div[@class='product-gallery']//img")]
            print('images.....', images)
        except (AttributeError, TypeError, IndexError, NoSuchElementException):
            images = ''
        try:
            upc_details = [part_model.text.strip() for part_model in self.driver.find_elements(By.XPATH, "//div[@class='data-models__col data-sheet__col--title js-edit-popup-name']")]
            upc_number = {'models': upc_details}
        except (NoSuchElementException, ElementClickInterceptedException, AttributeError, TypeError, IndexError):
            upc_number = ''
        ...
        # --------------------------------------------- specifications_1 -----------------------------------------------
        try:
            attr_name = [table.text.strip() for table in self.driver.find_elements(By.XPATH, "//*[@class='product__cell product__label']")]
            attr_value = [table.text.strip() for table in self.driver.find_elements(By.XPATH, "//*[@class='product__cell product__val']")]
            specs_li = [{name_li: values_li} for name_li, values_li in zip(attr_name, attr_value)]
        except (NoSuchElementException, ElementClickInterceptedException, AttributeError, TypeError, IndexError):
            specs_li = ''

        try:
            self.driver.find_element(By.XPATH, "//span[normalize-space()='Specs']").click()
            time.sleep(1)
        except NoSuchElementException:
            pass

        try:
            attr_name_1 = [table1.text.strip() for table1 in self.driver.find_elements(By.XPATH, "//*[@class='spec-list__label']")]
            attr_value_1 = [table1.text.strip() for table1 in self.driver.find_elements(By.XPATH, "//*[@class='spec-list__value']")]
        ...
        raw_data = [{
            "brand": "Vollrath", "catlvl1": catlvl1, "catlvl2": catlvl2, "catlvl3": catlvl3, "url": self.url,
...
# scaper_name = WebScraper('https://www.partstown.com/vollrath/vol25987-1')
...
def ReadUrlFromList():
    file_path = "D:/Web-Scrapping/A-MONTH-2024/May/Partstown.com/url/Product-url.csv"
    url_links = pd.read_csv(file_path)['URL']
    # print(url_links)
    i = 0
    for url_count, url in enumerate(url_links[i:50], start=i):
        urls.append(url)
