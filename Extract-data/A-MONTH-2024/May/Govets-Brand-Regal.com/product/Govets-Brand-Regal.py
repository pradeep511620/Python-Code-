...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        # -------------------------------------------------- BreadCrumb ------------------------------------------------
        try:
            names = [l3.text.strip() for l3 in self.pk.select(".breadcrumbs .items li")]
            l3_Name = "## ".join(names)
        ...
        # -------------------------------------------------- Title -----------------------------------------------------
        try:
            product_title = self.pk.select_one(".page-title").text.strip()
        ...
        # --------------------------------------------------- MPN ------------------------------------------------------
        try:
            mpn = self.pk.select_one("#product_info").text.strip().replace("Brand: RegalPart #: ", "")
            print('mpn.....', mpn)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            mpn = 'N/A'
            print('mpn:', mpn)
        ...
        # ---------------------------------------------- Description_1 -------------------------------------------------
        try:
            short = self.pk.select_one(".descriptionsssssss").text.strip()
            short_desc = short
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            short_desc = ''
        ...
        # ------------------------------------------------ Miscellaneous -----------------------------------------------
        try:
            Discount = self.pk.select("#pricing_info span")[0].text.strip().replace("Discount: ", "")
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            Discount = ''
            print(f"An error occurred: {e}\t{Discount}")

        try:
            Shipping = self.pk.select("#pricing_info span")[1].text.strip()
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            Shipping = ''
            print(f"An error occurred: {e}\t{Shipping}")

        try:
            logo = self.pk.select_one(".ambanners.ambanner-7 p img").get('src')
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            logo = ''
        ...
        try:
            sku_details = self.pk.select_one(".product.attribute.sku .value").text.strip()
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            sku_details = ''
