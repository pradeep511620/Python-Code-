...
save_files = open("D:/Web-Scrapping/A-MONTH-2024/May/Gordonelectricsupply-Brand-Superstrut/data/Gordonelectricsupply-Brand-Superstrut.csv", "a+", encoding="utf-8")
...
class WebScraper:
    def __init__(self, url):
        self.url = url
        # self.driver = self.SendDriverOnly()
    ...
    def Product_Details(self):
        ...
        # -------------------------------------------------- BreadCrumb ------------------------------------------------
        try:
            names = [l3.text.strip() for l3 in self.pk.select("#breadcrumb .breadcrumblink")]
            l3_Name = "## ".join(names)
        ...
        # -------------------------------------------------- Title -----------------------------------------------------
        try:
            product_title = self.pk.select_one(".main-content h1").text.strip()
        ...
        try:
            list_of_price = self.pk.select_one('.price-box').get_text(strip=True, separator=">>>").replace("Your Price:>>>", "").replace(">>>Total Available:>>>In Stock", "")
        ...
        # --------------------------------------------------- MPN ------------------------------------------------------
        try:
            mpn = self.pk.select(".description-box p")[-1].text.strip().replace("Part Number: ", "")
            print('SKU.....', mpn)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            mpn = 'N/A'
            print('SKU:', mpn)
        ...
        # ---------------------------------------------- Description_1 -------------------------------------------------
        try:
            short = self.pk.select_one(".description").text.strip()
            short_desc = short
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            print(f"An error occurred: {e}")
            short_desc = ''
        #
        try:
            desc = [desc_text.text.strip() for desc_text in self.pk.select_one(".description-box p")]
        ...
        # -------------------------------------------------- Images ----------------------------------------------------
        try:
            images = [{"src": img.get('src'), "alt": img.get('alt')} for img in self.pk.select('[name="productBox"] .left-section img')]
            print('images.....', images)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            print(f"An error occurred: {e}")
            images = ''
        ...
        try:
            ...
            attr_name = []
            attr_value = []
            for table_row in self.pk.select("#spectable tbody tr td"):
                row_count += 1
        ...
        try:
            availability = ''
            price_box = self.pk.find('div', class_='price-box')
            if price_box:
                total_availability = price_box.find('span', class_='total-availability')
                if total_availability:
                    stock_status = total_availability.next_sibling
                    availability = {"availability": stock_status}
                else:
                    print("Total availability span not found")
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            availability = ''
            print(f"An error occurred: {e}\t{availability}")




        try:
            {"Category": self.pk.select_one(".product_meta .posted_in a").text.strip()}
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            Category = ''
            print(f"An error occurred: {e}\t{Category}")

        miscellaneous = str(availability)
...
# scaper_name = WebScraper('https://www.gordonelectricsupply.com/p/S-Strut-Sec-A-Channel-End-Cap/5999584?ID=/ABB-Inc-Thomas-Betts/T-B-Superstrut/mfr-1IC5')
...
def ReadUrlFromList():
    file_path = "D:/Web-Scrapping/A-MONTH-2024/May/Gordonelectricsupply-Brand-Superstrut/url/Product-url.csv"
