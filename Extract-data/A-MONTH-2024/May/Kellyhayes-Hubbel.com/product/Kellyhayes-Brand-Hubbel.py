...
save_files = open("D:/Web-Scrapping/A-MONTH-2024/May/Kellyhayes-Hubbel.com/data/Hubbel.csv", "a+", encoding="utf-8")
base_url = ''
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        # ---------------------------------------------- Description_1 -------------------------------------------------
        try:
            short = self.pk.select_one(".product.attribute.overview .value").text.strip()
            short_desc = short
        except (NoSuchElementException, ElementClickInterceptedException, AttributeError, TypeError, IndexError):
            short_desc = ''
        #
        ...
        # -------------------------------------------------- Images ----------------------------------------------------
        try:
            images = [{"src": img.get('src'), "alt": img.get('alt')} for img in self.pk.select(".fotorama__stage img")]
            print('images.....', images)
        except (NoSuchElementException, ElementClickInterceptedException, AttributeError, TypeError, IndexError, Exception) as e:
            print(f"An error occurred: {e}")
            images = ''
        # print(self.pk.select(".fotorama__img"))
        # --------------------------------------------- specifications_1 -----------------------------------------------
        try:
        ...
        # ------------------------------------------------ Miscellaneous -----------------------------------------------
        try:
            MANUFACTURER = {"MANUFACTURER": self.pk.select_one(".product.attibute.manufacturer_name .value").text.strip()}
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            MANUFACTURER = ''
...
# scaper_name = WebScraper('https://www.kellyhayes.com/200lugb6-connector-200a-bi-metal-2-0-str.html')
...
def ReadUrlFromList():
    file_path = "D:/Web-Scrapping/A-MONTH-2024/May/Kellyhayes-Hubbel.com/url/Product-url.csv"
