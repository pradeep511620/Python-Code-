...
urls = []
save_files = open("D:/Web-Scrapping/A-MONTH-2024/May/Westechequipment.com/data/Westechequipment.csv", "a+", encoding="utf-8")
base_url = 'https://westechequipment.com'
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        try:
            names = [l3.text.strip() for l3 in self.soup.find(class_="small-text breadcrumb-container").find_all('a')]
            l3_Name = "## ".join(names)
        ...
        # --------------------------------------------------------------------------------------------------------------
        try:
            product_title = self.soup.find(class_="m-0 pb-4").find('h5').get_text(strip=True, separator=" ")
        ...
        # -------------------------------------------------- Price-- ---------------------------------------------------
        try:
            product_price_1 = self.soup.find(class_="table table-bordered product-table").find('tbody').find_all('td')[1].get_text(strip=True)
        ...
        # --------------------------------------------------- MPN ------------------------------------------------------
        try:
            mpn = self.soup.find(class_="table table-bordered product-table").find('tbody').find('th').get_text(strip=True)
            print('SKU.....', mpn)
        except (NoSuchElementException, ElementClickInterceptedException, AttributeError, TypeError, IndexError):
            mpn = 'N/A'
            print('SKU:', mpn)
        ...
        try:
            short = self.soup.find(class_="m-0 pb-4").find('p').get_text(strip=True)
            short_desc = short
        except (NoSuchElementException, ElementClickInterceptedException, AttributeError, TypeError, IndexError):
            short_desc = ''

        try:
            desc = [desc_text.get_text(strip=True, separator=">>").split(".>>")[0] for desc_text in self.soup.find(class_="tab-content mb-3")]
        ...
        # ---------------------------------------------- Description_2 -------------------------------------------------
        try:
            description_2 = [des_1.text.strip() for des_1 in self.soup.find(class_="tab-content mb-3").find_all('li')]
        ...
        # -------------------------------------------------- Images ----------------------------------------------------
        try:
            images = [{"src": urljoin(base_url, img.get('src')), "alt": img.get('alt')} for img in self.soup.find(class_="product-page-image-container pb-3").find_all('img')]
            print('images.....', images)
        except (AttributeError, TypeError, IndexError, NoSuchElementException):
            images = ''
        ...
        try:
            attr_value = ''
            attr_name = [table.text.strip() for table in self.soup.find(class_="bg-dark-grey").find_all('th')]
            # attr_value = [table.get_text(strip=True, separator=">>").split('>>') for table in self.soup.find(class_="table table-bordered product-table").find('tbody').find_all('tr')]
            for table in self.soup.find(class_="table table-bordered product-table").find('tbody').find_all('tr'):
                attr_value = table.get_text(strip=True, separator=">>").split('>>')
        ...
        raw_data = [{
            "brand": "Coxreel", "catlvl1": catlvl1, "catlvl2": catlvl2, "catlvl3": catlvl3, "url": self.url,
...
# scaper_name = WebScraper('https://westechequipment.com/product/1185-2028-a-4-gast-air-motor-rewind-oil-hose-reel')
...
def ReadUrlFromList():
    file_path = "D:/Web-Scrapping/A-MONTH-2024/May/Westechequipment.com/url/Product-url.csv"
