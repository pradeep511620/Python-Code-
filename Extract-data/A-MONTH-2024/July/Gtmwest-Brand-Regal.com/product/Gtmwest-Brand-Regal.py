...
save_files = open("P:/Web-scrapings/A-MONTH-2024/July/Gtmwest-Brand-Regal.com/data/Gtmwest-Brand-Regal.csv", "a+", encoding="utf-8")
base_url = 'https://gtmwest.com/'
headers = {
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        # -------------------------------------------------- BreadCrumb ------------------------------------------------
        try:
            l3_name_list = [l3.text.strip() for l3 in self.pk.select("ul.cimm_breadcrumbs li a")]
        ...
        # -------------------------------------------------- Title -----------------------------------------------------
        try:
            product_title = self.pk.select_one("h4.cimm_prodDetailTitle").text.strip()
        ...
        try:
            list_of_price = self.pk.select_one("div.cimm_itemPriceAvailabilWrap #yourPrices [data-select='priceData']").text.strip()
        ...
        try:
            ...
            short = self.pk.select_one("div.cimm_itemDescription p.cimm_itemShortDesc").text.strip()
            short_desc = short
        except (AttributeError, TypeError, IndexError, Exception) as e:
            short_desc = ''
        ...
        try:
            desc = [des.text.strip() for des in self.pk.select("div#descriptionSection p")]
        ...
        # -------------------------------------------------- Images ----------------------------------------------------
        try:
            images = [{"src": img.get('src'), "alt": img.get('alt')} for img in self.pk.select("div.cimm_itemMainImage img")]  # if "svg" not in img.get('src', '')
            print('images.....', images)
        except (AttributeError, TypeError, IndexError, Exception) as e:
            images = ''
        ...
        specs = ''
        try:
            attr_name = [th.text.strip() for th in self.pk.select("div#specificationSection table tr td b")]
            attr_value = [td.text.strip() for td in self.pk.select("div#specificationSection table tr td span")]
            attr_value = [value for value in attr_value if value != ':']
        ...
        # ------------------------------------------------- Realated ---------------------------------------------------
        try:
            related_url = [urljoin(base_url, href_link.get('href')) for href_link in self.pk.select("div.ProductCard-module_productContainer__3SBeF .clearAfter a.cimm_prodDetailTitle")]
            if not related_url:
        ...
        # ------------------------------------------------ Miscellaneous -----------------------------------------------
        try:
            part_number = self.pk.select_one("li#sitePartNo span").text.strip()
        except (AttributeError, TypeError, IndexError, Exception) as e:
            part_number = ''
            print(f"part_number.....An error occurred: {e}\t{part_number}")

        try:
            Availability = self.pk.select_one("span[data-select='availability']").text.strip()
        except (AttributeError, TypeError, IndexError, Exception) as e:
            Availability = ''
            print(f"Availability.....An error occurred: {e}\t{Availability}")


        miscellaneous = {"part_number": part_number, "Availability": Availability}
...
# scraper_names = WebScraper('https://www.gtmidwest.com/2366154/Product/Regal-030400TC')
...
# ----------------------------------------------------------------------------------------------------------------------
def main():
    file_path = "P:/Web-scrapings/A-MONTH-2024/July/Gtmwest-Brand-Regal.com/url/Product-url.csv"
