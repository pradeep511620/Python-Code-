...
save_files = open("P:/Web-scrapings/A-MONTH-2024/July/Distributionarc-Brand-Johnson-controls.com/data/Distributionarc-Brand-Johnson-controls.csv", "a+",encoding="utf-8")
base_url = 'https://www.Distributionarc.com/'
headers = {
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        # -------------------------------------------------- BreadCrumb ------------------------------------------------
        try:
            l3_name_list = [l3.text.strip().replace("/##", "") for l3 in self.pk.select(".breadcrumb_links span")]
            l3_list = [item for item in l3_name_list if item != '/']
            l3_Name = "## ".join(l3_list)
        ...
        # -------------------------------------------------- Title -----------------------------------------------------
        try:
            product_title = self.pk.select_one(".product__title h1").text.strip().replace('\n', ">>").replace(" ","").replace(">>>>>>>>", ' ')
        ...
        # ---------------------------------------------- Description_1 -------------------------------------------------
        try:
            short = self.pk.select_one(".col-md-12.innerintroTable .rfq1").text.strip()
            short_desc = short
        except (AttributeError, TypeError, IndexError, NoSuchElementException, Exception) as e:
            short_desc = ''
        ...
        try:
            desc = [des_div.text.strip() for des_div in self.pk.select(".data_disc p")]
        ...
        try:
            attr_name = []
            attr_value = []
            for row in self.pk.select(".table.table-bordered tbody tr"):
        ...
        # ------------------------------------------------- Realated ---------------------------------------------------
        try:
            related_url = [urljoin(base_url, href_link.get('href')) for href_link in self.pk.select(".related_components_ul a")]
            if not related_url:
        ...
        miscellaneous = {"Manufacturer_element": Brand, "Sub_Category": Sub_Category, 'Category': Category}
...
# scraper_names = WebScraper('https://www.distributionarc.com/rfq/product/hvac/johnson-controls/vwe020hb-92nbga/')
...
# ----------------------------------------------------------------------------------------------------------------------
def main():
    file_path = "P:/Web-scrapings/A-MONTH-2024/July/Distributionarc-Brand-Johnson-controls.com/url/Product-url - Copy.csv"
