...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        # --------------------------------------------------- MPN ------------------------------------------------------
        try:
            mpn = self.pk.select_one("p.product__sku").text.strip().replace('SKU:Code: ', '')
            print('SKU.....', mpn)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            mpn = ''
        ...
        try:
            desc = [des_div.text.strip() for des_div in self.pk.select("pdp-tab .description p")]
        ...
        try:
            attr_name = []
            attr_value = []
            for row in self.pk.select(".specs ul li"):
...
# scraper_names = WebScraper('https://www.hroberts-di.com/collections/mitutoyo/products/mitutoyo-543-701b-absolute-digimatic-digital-indicator-id-cnx-range-0-5-12-7mm-resolution-0-0005-0-0001-0-00005-0-00002-0-01mm-0-001mm-0-0005mm-selectable')
