...
save_files = open("P:/Web-scrapings/A-MONTH-2024/July/Eldonjames-Products.com/data/Eldonjames-Products.csv", "a+", encoding="utf-8")
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        # -------------------------------------------------- BreadCrumb ------------------------------------------------
        try:
            l3_name_list = [l3.text.strip() for l3 in self.pk.select("nav.woocommerce-breadcrumb a")]
        ...
        # -------------------------------------------------- Title -----------------------------------------------------
        try:
            product_title = self.pk.select_one("h1.product_title").text.strip()
        ...
        # --------------------------------------------------- MPN ------------------------------------------------------
        try:
            mpn = self.pk.select_one("div.product_meta .sku").text.strip()
            print('SKU.....', mpn)
        except (AttributeError, TypeError, IndexError, Exception) as e:
            mpn = ''
        ...
        # -------------------------------------------------- Images ----------------------------------------------------
        try:
            images = [{"src": img.get('src'), "alt": img.get('alt')} for img in self.pk.select("div.product-gallery img")]  # if "svg" not in img.get('src', '')
            print('images.....', images)
        except (AttributeError, TypeError, IndexError, Exception) as e:
            images = ''
        ...
        try:
            category = [cate.text.strip() for cate in self.pk.select("span.posted_in a")]
        except (AttributeError, TypeError, IndexError, Exception) as e:
            category = []
            print(f"MFR.....An error occurred: {e}\t{category}")
        ...
        miscellaneous = {"categories": category, "tags": tags}
...
# scraper_names = WebScraper('https://eldonjames.com/product/lslr-pp-b-qc/')
