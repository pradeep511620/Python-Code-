...
save_files = open("P:/Web-scrapings/A-MONTH-2024/July/I-c-e-inc-Brand-Anvil.com/data/I-c-e-inc-Brand-Anvil.csv", "a+", encoding="utf-8")
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        catlvl3 = ''
        try:
            Brand = self.pk.select_one('h3.cimm_prodDetailBrand').text.strip().replace('Brand Name: ', '')
            print('Brand.....', Brand)
        ...
        # -------------------------------------------------- BreadCrumb ------------------------------------------------
        try:
            l3_name_list = [l3.text.strip() for l3 in self.pk.select("ul.cimm_breadcrumbs li")]
        ...
        # -------------------------------------------------- Title -----------------------------------------------------
        try:
            product_title = self.pk.select_one(".cimm_prodDetailTitle").text.strip()
        ...
        # -------------------------------------------------- Price -----------------------------------------------------
        try:
            qty = self.pk.select_one("#yourPrices em").text.strip()
        ...
        try:
            list_of_price = self.pk.select_one("#yourPrices span").text.strip()
        ...
        # --------------------------------------------------- MPN ------------------------------------------------------
        try:
            mpn = self.pk.select_one("#mPartNo span").text.strip()
            print('SKU.....', mpn)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            mpn = ''
        ...
        # ---------------------------------------------- Description_1 -------------------------------------------------
        try:
            short = self.pk.select_one("p.cimm_itemShortDesc").text.strip()
            short_desc = short
        except (AttributeError, TypeError, IndexError, NoSuchElementException, Exception) as e:
            short_desc = ''
        ...
        try:
            desc = [des_div.text.strip() for des_div in self.pk.select("#descriptionSection p")]
        ...
        # ---------------------------------------------- Description_2 -------------------------------------------------
        try:
            description_2 = [find_div.get_text(strip=True, separator=">>") for find_div in self.pk.select("#featureSection ul li")]
        ...
        # -------------------------------------------------- Images ----------------------------------------------------
        try:
            images = [{"src": urljoin(base_url, img.get('src')), "alt": img.get('alt')} for img in self.pk.select(".cimm_itemMainImage img")]
            print('images.....', images)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            images = ''
        ...
        try:
            attr_name = []
            attr_value = []
            for row in self.pk.select("#specificationSection table tbody tr"):
        ...
        # ------------------------------------------------ Datasheet ---------------------------------------------------
        # try:
        #     datasheet = [urljoin(base_url, pdf.get('onclick')) for pdf in self.pk.select("#documentsSection a") if 'pdf' in pdf.get('onclick', '')]
        #     if not datasheet:
        ...
        try:
            datasheet = []
            links = self.pk.select("#documentsSection a")
            for link in links:
                onclick_attr = link.get('onclick')
                if onclick_attr and 'pdf' in onclick_attr:
                    pdf_url = onclick_attr.split("'")[1]
                    full_pdf_url = urljoin(base_url, pdf_url)
                    datasheet.append(full_pdf_url)
            if not datasheet:
                raise ValueError("No datasheet found")
            print('Datasheet:', datasheet)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
        ...
        # ------------------------------------------------ Miscellaneous -----------------------------------------------
        try:
            stock_available = self.pk.select_one("#minOrdQty span").text.strip()
        except (AttributeError, TypeError, IndexError, Exception) as e:
            stock_available = ''
            print(f"An error occurred: {e} stock_available -----: {stock_available}")

        try:
            qtyInt = self.pk.select_one("#qtyInt span").text.strip()
        except (AttributeError, TypeError, IndexError, Exception) as e:
            qtyInt = ''
            print(f"An error occurred: {e} qtyInt -----: {qtyInt}")

        try:
            ICEPN = self.pk.select_one("#sitePartNo span").text.strip()
        except (AttributeError, TypeError, IndexError, Exception) as e:
            ICEPN = ''
            print(f"An error occurred: {e} ICEPN -----: {ICEPN}")

        miscellaneous = {"ICEPN": ICEPN, "qtyInt": qtyInt, "stock_available": stock_available}
...
# scraper_names = WebScraper('https://www.i-c-e-inc.com/2334368/Product/Anvil%C2%AE-0500361068-0500361068')
...
# ----------------------------------------------------------------------------------------------------------------------
def main():
    file_path = "P:/Web-scrapings/A-MONTH-2024/July/I-c-e-inc-Brand-Anvil.com/url/Product-url.csv"
