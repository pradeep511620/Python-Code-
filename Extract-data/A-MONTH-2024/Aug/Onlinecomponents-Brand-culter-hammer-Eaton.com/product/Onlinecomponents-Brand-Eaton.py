...
save_files = open("P:/Web-scrapings/A-MONTH-2024/Aug/Onlinecomponents-Brand-culter-hammer-Eaton.com/data/Onlinecomponents-Brand-culter-hammer-Eaton.csv", "a+",encoding="utf-8")
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        # ------------------------------------------------ Datasheet ---------------------------------------------------
        try:
            datasheet = datasheet = [pdf.get('href') for pdf in self.pk.select("div#ResourceTable li a") if pdf.get('href') and pdf.get('href').startswith('https')]
            if not datasheet:
        ...
        try:
            Out_of_stock = self.pk.select('#trInstock span')[0].get_text(strip=True, separator=' ')
            # print(Out_of_stock)
        except (AttributeError, TypeError, IndexError, Exception) as e:
            Out_of_stock = ''
...
# scraper_names = WebScraper('https://www.onlinecomponents.com/en/productdetail/cutler-hammer--eaton/xtpext032b-56196481.html')
# scraper_names.Product_Details()


# ----------------------------------------------------------------------------------------------------------------------
def main():
    file_path = "P:/Web-scrapings/A-MONTH-2024/Aug/Onlinecomponents-Brand-culter-hammer-Eaton.com/url/Product-url.csv"
