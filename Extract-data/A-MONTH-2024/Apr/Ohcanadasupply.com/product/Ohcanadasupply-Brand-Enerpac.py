...
urls = []
save_files = open("C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Apr/Ohcanadasupply.com/data/Ohcanadasupply.csv", "a+",
                  encoding="utf-8")
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        try:
            ...
            catlvl3 = ''
            try:
                bread_cat = self.driver.find_element(By.XPATH, "//span[@class='breadcrumb--truncate']").text.strip()
            except (AttributeError, TypeError, IndexError, NoSuchElementException):
                bread_cat = ''
            try:
                names = []
                for l3 in self.driver.find_elements(By.XPATH, "//*[@class='breadcrumb']//a"):
                    l3_details = l3.text.strip().replace("/", "")
                    if l3_details:
                        names.append(l3_details)
                l3_Name = "## ".join(names) + '## ' + bread_cat
            ...
            # --------------------------------------------------------------------------------------------------------------
            try:
                product_title = self.driver.find_element(By.XPATH, "//h1[@class='product-title']").text.strip()
            ...
            # --------------------------------------------------- MPN ------------------------------------------------------
            try:
                mpn = self.driver.find_element(By.XPATH, "(//*[@ng-bind='mfrNumber'])[1]").text.strip()
                print('mpn.....', mpn)
            except NoSuchElementException:
                mpn = 'N/A'
                print('mpn:', mpn)
            ...
            # ---------------------------------------------- Description_1 -------------------------------------------------
            try:
                des = [desc .text.strip() for desc in self.driver.find_elements(By.XPATH, "(//*[@ng-bind-html='descr.content'])[1]//p")]
            ...
            # ---------------------------------------------- Description_2 -------------------------------------------------
            try:
                description_2 = [des_1.text.strip() for des_1 in self.driver.find_elements(By.XPATH, "//div[@itemprop='description']//ul//li")]
            ...
            try:
                images = []
                for images_html in self.driver.find_elements(By.XPATH, "//div[@id='productPhoto']//img"):
                    if images_html:
            ...
            # --------------------------------------------------------------------------------------------------------------
            try:
                attr_name = [th.text.strip() for th in self.driver.find_elements(By.XPATH, "(//*[@class='properties'])[1]//dt")]
                attr_value = [td.text.strip() for td in self.driver.find_elements(By.XPATH, "(//*[@class='properties'])[1]//dd")]
            ...
            try:
                related_url = []
                for href in self.driver.find_elements(By.XPATH, "//div[@class='grid-uniform']//a"):
            ...
            try:
                free_shipping = self.driver.find_element(By.XPATH, "//*[@data-target='#collapseFreeShipping']//img").get_attribute('src')
            except (NoSuchElementException, IndexError, TypeError):
                free_shipping = ''


            miscellaneous = free_shipping
...
sss = 'https://ohcanadasupply.ca/Tools-Shop-Equipment-and-Storage/Hand-Tools/Pullers-and-Separators/enpfsm8'
scaper_name = WebScraper(sss)
...
#
# def ReadUrlFromList():
#     file_path = "C:/Users/PK/Desktop/web-scrapping/A-MONTH-2024/Apr/Ohcanadasupply.com/url/Product-url.csv"
