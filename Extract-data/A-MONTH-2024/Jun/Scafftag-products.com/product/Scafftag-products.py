...
save_files = open("P:/Web-scrapings/A-MONTH-2024/Jun/Scafftag-products.com/data/Scafftag-products.csv", "a+", encoding="utf-8")
base_url = ''
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        # -------------------------------------------------- Title -----------------------------------------------------
        try:
            product_title = self.pk.select_one("[itemprop='name']").text.strip()
        ...
        # --------------------------------------------------- MPN ------------------------------------------------------
        try:
            mpn = self.pk.select_one("[itemprop='sku']").text.strip().replace("EDP: ", "")
            print('SKU.....', mpn)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            mpn = ''
        ...
        # ---------------------------------------------- Description_1 -------------------------------------------------
        try:
            short = self.pk.select_one("[itemprop='description']").text.strip()
            short_desc = short
        except (AttributeError, TypeError, IndexError, NoSuchElementException, Exception) as e:
            short_desc = ''
        ...
        try:
            ...
            desc = [des_div.text.strip() for des_div in self.pk.select(".product.attribute.description p")]
        ...
        # -------------------------------------------------- Images ----------------------------------------------------
        try:
            images = [{"src": img.get('src'), "alt": img.get('alt')} for img in self.pk.select(".product.media img") ]
            print('images.....', images)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            images = ''
        ...
        # --------------------------------------------- specifications_1 -----------------------------------------------
        try:
            attr_name2 = [th.text.strip() for th in self.pk.select('#product-attribute-specs-table tbody tr th')]
            attr_value2 = [td.text.strip() for td in self.pk.select("#product-attribute-specs-table tbody tr td")]
...
# scraper_names = WebScraper('https://www.scafftag.co.uk/man-in-tank-inserts-eitl-529.html')
...
def main():
    file_path = "P:/Web-scrapings/A-MONTH-2024/Jun/Scafftag-products.com/url/Product-url.csv"
