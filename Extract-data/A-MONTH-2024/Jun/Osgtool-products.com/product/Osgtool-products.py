...
save_files = open("P:/Web-scrapings/A-MONTH-2024/Jun/Osgtool-products.com/data/Osgtool-products.csv", "a+", encoding="utf-8")
base_url = ''
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        # -------------------------------------------------- BreadCrumb ------------------------------------------------
        try:
            names = [l3.text.strip() for l3 in self.pk.select(".productView-list-brand a")]
            l3_Name = "## ".join(names)
        ...
        # -------------------------------------------------- Title -----------------------------------------------------
        try:
            product_title = self.pk.select_one('.custom-remove-pdf.category__header h1').text.strip()
        ...
        try:
            list_of_price = self.pk.select_one(".warning--login-for-princing").text.strip()
        ...
        # --------------------------------------------------- MPN ------------------------------------------------------
        try:
            mpn = self.pk.select_one(".productView-product__id").text.strip().replace("EDP: ", "")
            print('SKU.....', mpn)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            mpn = ''
        ...
        try:
            ...
            desc = self.pk.select(".productView-info-text")[0].text.strip()
            # if not desc:
        ...
        # -------------------------------------------------- Images ----------------------------------------------------
        try:
            images = [{"src": img.get('src'), "alt": img.get('alt')} for img in self.pk.select(".productView-images img") if 'youtube' not in img.get('src', '')]
            print('images.....', images)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            images = ''
        ...
        try:
            ...
            attr_value2 = []
            count = 0
            for table in self.pk.select("#custom-product-dimension-table tbody tr td"):
                count += 1
        ...
        try:
            s_list = []
            ins_list = []
            cart_details_list = self.pk.select('.card-stock.card-stock--inStock p')
        ...
        try:
            brands = {"brands": self.pk.select_one(".productView-product__name").text.strip()}
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            brands = ''
            print(f"An error occurred: {e} brands -----: {brands}")

        miscellaneous = stock_data, brands
...
# scraper_names = WebScraper('https://osgtool.com/79961/')
...
def ReadUrlFromList():
    file_path = "P:/Web-scrapings/A-MONTH-2024/Jun/Osgtool-products.com/url/Product-url.csv"
