...
save_files = open("D:/Web-Scrapping/A-MONTH-2024/Jun/Weg-products.com/data/weg-products.csv", "a+", encoding="utf-8")
base_url = ''
...
class WebScraper:
    ...
    def Product_Details(self):
        ...
        # -------------------------------------------------- Title -----------------------------------------------------
        try:
            product_title = self.pk.select_one('.pb10 h1').text.strip()
        ...
        # --------------------------------------------------- MPN ------------------------------------------------------
        try:
            mpn = self.pk.select_one('.product-card-info').text.strip()
            print('SKU.....', mpn)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            mpn = ''
        ...
        try:
            ...
            desc = self.pk.select_one('.xtt-product-description p').text.strip()
            # if not desc:
        ...
        # -------------------------------------------------- Images ----------------------------------------------------
        try:
            images = [{"src": img.get('src'), "alt": img.get('alt')} for img in self.pk.select(".col-xs-12.col-md-6 li img")]
            print('images.....', images)
        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            images = ''
        ...
        # --------------------------------------------- specifications_1 -----------------------------------------------
        try:
            attr_name1 = [th.text.strip() for th in self.pk.select(".row.product-info-specs .table tbody tr th")]
            attr_value1 = [td.text.strip() for td in self.pk.select(".row.product-info-specs .table tbody tr td")]
            specs1 = [{name: value} for name, value in zip(attr_name1, attr_value1)]

        except (NoSuchElementException, AttributeError, TypeError, IndexError, Exception) as e:
            specs1 = []
            print(f"An error occurred: {e} specifications_1 -----: {specs1}")

        try:
            attr_name = [th.text.strip() for th in self.pk.select(".product-techspecs-tabs table tbody tr th")]
            attr_value = [td.text.strip() for td in self.pk.select(".product-techspecs-tabs table tbody tr td")]
        ...
        specifications_1 = str(specs) + ", " + str(specs1)
        ...
        # ------------------------------------------------ Datasheet ---------------------------------------------------
        try:
            datasheet = [pdf.get('href') for pdf in self.pk.select(".tab-content .list-unstyled li a")]
...
scaper_name = WebScraper('https://www.weg.net/catalog/weg/US/en/Digital-Solutions/Condition-Monitoring/WEG-Motion-Fleet-Management/WEG-Motion-Fleet-Management/p/80002641')
